# Load libraries
packs <- c("tidyverse","imager", "colorscience", "randomForest", "doParallel")
...
fit_yam <- readRDS("./out/YamPixelClassifier.rds")

# Applying the function to all jpg files in a directory
alldata <- list.files(path="./out/CroppedFrame/", patt='.png', ignore.case=TRUE)
...
temp<-foreach(i=1:length(alldata), .verbose=T,
    ...
    pic<-load.image(paste0("./out/CroppedFrame/", afile))
    dim1<-dim(pic)
    mat<-channels(pic)
...
bdf$pRec<-as.numeric(bdf$pRec)
write.csv2(bdf, "./out/Yam_pRec.csv", col.names=T, sep=";", row.names=F)
